...
export default function HomePage() {
  ...
  const checkPin = async () => {
    try {
      const result = await contractInstance.methods.enterPin(pin).call();
      setMessage(result);
    } catch (error) {
      console.error(error);
      setMessage('Error occurred while checking pin');
    }
}
    ...
    return (
      <>
        <div>
          ...
          <p>Your Pin is:{message}</p>
  
          <button style={{ backgroundColor: "#cyan" }} onClick={deposit}>
            Deposit 1 ETH
          </button>
          <button style={{ backgroundColor: "green" }} onClick={withdraw}>
            Withdraw 1 ETH
          </button>
        </div>
